...
def get_interpolation_expressions(x, y, start_cond, end_cond, lbda):
    n_points = len(x)+2
    solutions = [[sp.symbols(f'solutions_{i}_{j}', real = True) for j in range( 6)] for i in range(n_points - 1)] #use as_mutable to get list
    ...
    y.insert(1, y_is)
    y.insert(-1, y_ie)
    potentials = [1, lbda, lbda**2, lbda**3, lbda**4, lbda**5]

    polynomes = [sum([solutions[i][j] * potentials[j] for j in range(6)]) for i in range(n_points - 1)]
    problems = []
    problems += [polynomes[0].subs(lbda,x[0]) - y[0],
                  polynomes[0].subs(lbda,x[1]) - y[1]]

    problems += [polynomes[0].diff(lbda,j).subs(lbda,x[0])
                -start_cond[j] for j in range(1, 4)]

    for i in range(1, n_points - 1):
        problems += [polynomes[i].subs(lbda,x[i]) - y[i],
                     polynomes[i].subs(lbda,x[i+1]) - y[i+1]]

        problems += [polynomes[i].diff(lbda,degree).subs(lbda,x[i])
                    -polynomes[i-1].diff(lbda,degree).subs(lbda,x[i])
                    for degree in range(1,5)]

    problems += [polynomes[n_points-2].diff(lbda,j).subs(lbda,x[n_points-1])
